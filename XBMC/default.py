...
# ShortcutRelayPy - Simple Discord Rich Presence script for XBMC4Xbox / XBMC4Gamers. For use with Mobcat/MrMilenko's "xbdStats" server.
...
SERVER_IP = '192.168.1.100' # Change this to your PC/server running server.py
SERVER_PORT = 1102 # Only change this if you've changed the port on your PC/server. 

# Ask the user to select the .XBE file for the game they'd like to run.
def select_xbe():
    dialog = xbmcgui.Dialog()
    xbe_path = dialog.browse(1, 'Select Game (Discord)', 'files', '.xbe')
    return xbe_path if xbe_path and os.path.isfile(xbe_path) else None

# Scan the .XBE for the title ID 
def read_titleid(xbe_path):
    try:
        with open(xbe_path, 'rb') as f:
            ...
            # 6. Return as 8-digit uppercase hex string
            return "%08X" % titleid
...
# Launch the game after sending the title ID
